...
class UnpinEquipmentToCabinetActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_unpin_equipment_to_cabinet)
        ...
        createCardList(database, this, adapter, cabinetId)

        val closeButton: ImageView = findViewById(R.id.closeButton)
        val unpinButton: FrameLayout = findViewById(R.id.unpinEquipmentButton)
    ...
    private fun assigning(database: AppDatabase, adapter: AssigningAndUnpinAdapter) {
        ...
        lifecycleScope.launch {
            database.equipmentDao().updateCabinetIdForNullOnEquipment(selectionList)
        }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                searchQuery
                    .debounce(150)
                    .flatMapLatest { text ->
                        database.equipmentDao().getEquipmentByCabinetIdFlow(cabinetId, text)
                    }
                    .collect { list ->
